---
name: video-pipeline
description: 短视频一条龙工作流。当用户说"做一条【类型/赛道】，主题是【XXX】"（类型可省略，自动推荐候选赛道）、"复刻这个链接：[链接]"，或提到旅行、个人成长、个人经历、往事、FPV、广告片、TVC、PV/PVC、宣传片、旅行宣传片、户外纪实、口播带货、种草，或要求短视频选题、分镜、豆包 Seedance/MiniMax Hailuo 3提示词、封面提示词、多平台发布包、@投放数据投后校准或视频数据复盘时使用。
---
...
- 收到`@投放数据`时：进入投后校准模式，读取当前或用户指定的workspace文件、[references/data-feedback.md](references/data-feedback.md)和[feedback-library.md](feedback-library.md)；禁止进入常规生成。
...
- **第三步·选配确认单（推荐+自定义）**：赛道和主题都确认后，禁止直接生成。必须输出【选配确认单】并立即挂起，逐项给出⭐推荐与自定义入口：
  ...
  - 风格：列出2-3种并标星推荐（如⭐快节奏打卡、生活流文艺）| 或自定义风格
  - 主角身份：列出2-3种并标星推荐（如⭐穷游大学生、精致打工人）| 或自定义身份
  - 生成模型：⭐Seedance 2.5 | 备选 Seedance 2.0 | 备选 Hailuo 3 | 或输入`模型=全部`；用户已显式指定模型时，确认单必须回显该选择
  - 目标平台：⭐推荐抖音、小红书 | 备选B站、视频号 | 或自定义平台
  - 末尾固定提示：`请回复您的选择（如：画幅=16:9，风格=快节奏打卡），或者直接输入您自定义的参数。或者回复"按推荐来"，我立刻生成。`
- **第四步·生成（严格挂起后一次性执行）**：仅在收到明确的赛道选择、主题选择、参数选择或自定义输入、"确认"、"按推荐来"指令后恢复执行；恢复后一口气输出选题策划、分镜脚本、视频生成提示词、封面提示词和发布包，中间不允许再次停顿。生成模型按选配确认单确认结果执行，未经确认不得切换；发布包只输出选配确认单中已确认的平台（该确认等价于平台名单确认），未经确认禁止输出任何平台代码块。输出完毕立即执行阶段落盘。
...
3. **视频生成提示词（参数化编译）**：按选配确认单确认的模型输出；用户可输入`模型=seedance|hailuo|全部`覆盖推荐项。若用户未预先指定，确认单默认推荐`Seedance 2.5`，用户回复“按推荐来”后等价确认`模型=seedance`，只输出1个代码块。`模型=seedance2.0`只输出 Seedance 2.0；`模型=hailuo`只输出 MiniMax Hailuo 3；`模型=全部`才输出三套。画幅固定默认9:16，仅用户明确要求时才输出16:9。每个代码块都覆盖固定叙事链、当前风格基线、人物连续性和镜头列表中的“穿越/跃迁”字段，音效必须分层递进，禁止“科技感、高级感”类空洞词。默认时长30秒；超过所选模型单段上限时按 [references/segment-split.md](references/segment-split.md) 的段数公式自动拆段，不需要用户手动拆。
...
- 无原片拆解模式（爆款结构逆向工程）：用户仅提供链接，未提供原片视频文件、完整关键帧序列或字幕/旁白文本时，禁止直接拒绝。第一行强制打印：`未检测到原片素材，将启用【爆款结构逆向工程】模式，提取其骨架为您生成全新脚本。`随后输出【骨架拆解表】，等待用户确认骨架和参数后才生成。
  - 骨架证据只使用用户消息中可见的标题、封面、简介、话题、账号类型或用户补充描述；不虚构为已看原片。若链接元数据不可见，按用户提供的新主题输出同赛道低置信结构假说，并在每行标注【推断】。
  - 骨架拆解表必须覆盖5个维度：①【开头钩子】判定为悬念型、冲突型、利益型或视觉型；②【情绪曲线】标记起、承、转、合，并预估高潮点在第几秒；③【叙事节奏】判定为快剪、慢叙事、口播带货或剧情反转；④【核心爆点】分析靠共鸣、搞笑、反差、干货或其他机制带来点赞；⑤【结尾引导】分析互动、关注、转化或评论引导方式。
  - 表末尾必须给出推荐赛道、主题、时长、画幅、风格、主角身份、平台和模型，并提示：`回复"确认骨架，按推荐来"生成全新脚本；或直接修改任意骨架/参数。`
  - 用户确认后，把骨架映射为新片结构，一次性输出选题策划、7列分镜脚本、按`模型=`选择的提示词、封面提示词和自查评分卡；发布包仍按平台确认规则执行。每段必须用全新主题、全新台词、全新画面，禁止照搬原视频人物名、台词、品牌、专有名名词、独特画面和镜头设计。
  - 重构前内部注入：`注意：此为逆向结构仿写，严禁照搬原视频画面描述，必须替换所有专有名词、视觉元素和对话，避免平台查重。`
  - 用户后续补充原片文件或完整关键帧+字幕时，可切换为逐秒对位复刻；有本地原片但素材不全时，先索要缺失部分再逐秒执行。
...
用户输入`@投放数据 播放量=5000 完播率=25% 点赞=300 评论=20`时，技能立即挂起常规生成，进入“数据校准模式”。缺失指标标【待补充】，禁止用估算值补齐。若用户未指定workspace且当前工作区无法唯一定位片子，先询问“校准哪一条？请回复 workspace 文件名或片名”；已能定位时只读workspace、[references/data-feedback.md](references/data-feedback.md)和[feedback-library.md](feedback-library.md)，禁止回读历史对话全文。

硬规则：

- 只要用户提供真实投放数据，此前生成的[自查评分卡]立即作废；真实数据是唯一投后评判标准，AI自评不得再作为质量结论。
- 点赞率=点赞÷播放量；评论率=评论÷播放量；互动率=(点赞+评论+收藏+分享)÷播放量。前3秒留人率只在用户给出分段数据时校准，否则标【待补充：前3秒留存】。
- 完播率<15%：强制判定“开场前3秒的钩子失效”，下一轮必须重写第1-3秒；同时检查00:03-00:10是否有事件推进，禁止归因为“平台不给量”。
- 点赞率>3%：强制判定“情绪共鸣或价值点做得好，应保留该结构”，并从workspace中写出可保留的镜号、台词或情绪峰值。
- 评论率>1%：强制判定“互动引导有效”，保留同类评论区钩子和结尾提问结构。
- 点赞率<1%且评论率<1%：判定情绪价值或互动钩子不足；若完播率也<15%，先修开场，再修中段事件密度。

每次投后校准必须依次输出：

```markdown
[数据校准模式] 自评评分卡已作废，以真实投放数据为唯一评判标准。

①【本次数据诊断】
<!-- 归一化指标、阈值判定、问题定位到具体镜号/时间码；缺workspace时标注【待补充：具体镜号】 -->

②【评分卡修正】
<!-- 用真实数据替换AI自评：完播率校准钩子强度，点赞率校准情绪价值，评论率校准互动引导；投后数据无法验证的维度写【投后不可直接校准】 -->

③【微调库写入】
<!-- 将“保留策略”和“避开坑”写成关键词与下次强制动作；确认已写入feedback-library.md -->

[投后校准承诺] 本技能后续生成该赛道脚本时，将优先检索 feedback-library.md 中的真实数据沉淀，而不是凭空套用模板。
```

微调库追加到 [feedback-library.md](feedback-library.md) 的【投放数据记录】，按赛道分组，每赛道只保留最近5条；每次记录必须包含片名、平台、指标、判定、关键词、下次强制动作。同一坑重复出现2次升级为硬禁令。下一轮同赛道生成前必须读取该库，命中记录时在选题策划开头打印：`[投后校准注入] 已读取XX赛道真实数据记录：保留XX，避开XX。`
